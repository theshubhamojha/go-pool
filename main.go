...
func (s *poolService) executeFunction(payload interface{}, taskId int) {
	now := time.Now()
	s.processor(payload)
...
func main() {
	...
	p := NewGoroutinePool(3, sampleFunction)
